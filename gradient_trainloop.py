...
def training_loop(input_size, num_classes, class_wise_data, class_dataloaders_test, save_path, lr = 0.001, num_samples = 32):
    ...
    # create a folder called save_path if it does not exist
    
    if not os.path.exists(save_path):
        os.makedirs(save_path)

        # make num_classes number of subfolders in save_path
    ...
    for cycle in range(max_cycles):
        ...
        for cla in range(num_classes):
            ...
            for inputs, labels in class_dataloaders[cla]:
                ...
                # get all gradients and save them to a file
                gradients = {name: param.grad for name, param in model.named_parameters() if param.grad is not None}
                for name, grad in gradients.items():
                    grad = grad.cpu().numpy()
                    np.save(os.path.join(save_path, str(cla), f"gradients_cycle_{cycle}_param_{name}.npy"), grad)
